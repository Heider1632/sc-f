<template>
  <v-container>
    <v-layout text-xs-center wrap grow>
        <!-- <v-flex xs12>
        ...
        <!-- Stopwatch Section -->
        <v-flex mb-4>
            <h1 class="display-1 font-weight-bold mb-3" v-html="formatTime(lapsedMilliseconds)"></h1>
            ...
            <v-card-text>
                ...
                <v-layout wrap>
                    ...
                    <div class="text-xs-center">
                        ...
                        <v-btn v-if="index < csvExportColumnOrder.length - 1" @click="swapArray(csvExportColumnOrder, index, index + 1)" text icon><v-icon>swap_horiz</v-icon></v-btn>
                        </span>
                    </div>
  ...
  export default {
    ...
    methods: {
      ...
      csvExport() {
        ...
        // Headers
        let headers = [];
        for (const [column] of Object.entries(this.csvExportColumnOrder)) {
          if (column.render) {
        ...
        // Body / Entries
        for (const [value] of Object.entries(this.stopwatchIntervals)) {
          let cells = [];
          for (const [column] of Object.entries(this.csvExportColumnOrder)) {
            if (column.render) {
