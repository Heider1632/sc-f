  ...
  <v-main>
    <v-container fill-height class="section-container">
      <v-layout row wrap>
                ...
                <v-layout row wrap>
                  <v-flex xs12 sm6>
                    <v-card class="section-card" flat color="primary">
                      ...
                      <v-card-text>
                        <p class="subtitle-2 white--text">Nombre: {{ user.name }} {{ user.lastname }}</p>
                        <p class="subtitle-2 white--text">
                          Estilo de Aprendizaje: {{ learningStyle(user.learningStyleDimensions) }}
                        </p>
                        <p class="subtitle-2 white--text">Correo: {{ user.email }}</p>
                        <p class="subtitle-2 white--text">Semestre: V</p>
                        <p class="subtitle-2 white--text">Programa: Enfermería</p>
                        <p class="subtitle-2 white--text">Insitución: Universidad de Cordoba</p>
                      </v-card-text>
                    </v-card>
                  </v-flex>
                </v-layout>
        ...
        <v-flex xs12 mb-8>
          <v-data-table :headers="headers" :items="data" disable-pagination hide-default-footer>
            <template v-slot:[`item.isActive`]="{ item }">
                    <v-chip
                        :color="getColor(item.isActive)"
                        dark
                    >
                        {{ item.isActive ? 'Completado' : 'En Progreso' }}
                    </v-chip>
            </template>
          </v-data-table>
          <v-toolbar dense flat >
            <v-toolbar-title>Tiempo Restante:</v-toolbar-title>
            <p class="display-5 mt-5 pa-3"> {{ total }} / 24 (info adicional: para completar esta experiencia de aprendizaje debes dedicar un mínimo de 24 horas)</p>
          </v-toolbar>
        </v-flex>
      </v-layout>
    </v-container>
...
export default {
  ...
  methods: {
    ...
    totalStudent(){
        this.$http.get('/trace/total/', { params: { student: this.user.student_id } }).then(response => {
            this.total = this.convertHMS(response.data.total);
        });
    },
    lessons(){
        this.$http.get('/progress/status/', { params: { student: this.user.student_id } }).then(response => {
            this.data = response.data.sort((a, b) => a.lesson.order - b.lesson.order);
        });
    },
